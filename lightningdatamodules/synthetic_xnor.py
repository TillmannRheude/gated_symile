...
from datasets.synthetic_xnor import SyntheticXNOR, SyntheticXNORBimodal
...
class DataModule_SyntheticXNORBimodal(pl.LightningDataModule):
    def __init__(
        self,
        batch_size: int = 128,
        n_samples: int = 100000,
    ...
    def setup(self, stage):
        self.ds = SyntheticXNORBimodal(
            ...
            n_bits=self.n_bits,
            embed_mode=self.embed_mode,
            ab_corr_exclusive=self.ab_corr_exclusive,
            ab_corr_p=self.ab_corr_p,
            ab_corr_split=self.ab_corr_split,
            p_flips=self.p_flips,
            p_corrs=self.p_corrs,
            corr_modes=self.corr_modes,
            signal_scale=self.signal_scale,
